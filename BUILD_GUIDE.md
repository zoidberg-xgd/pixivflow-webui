...
> ❌ **重要提示**：Electron 桌面应用功能目前**尚未实现**，仍在开发中。虽然可以构建应用，但功能不完整，不建议在生产环境使用。**请优先使用 Web UI 版本**。

本指南介绍如何构建 PixivFlow Electron 应用的所有平台版本（功能尚未完成）。
...
**当前状态**：Electron 桌面应用功能**尚未实现**，仍在积极开发中。
